...
{
	CRegKey regKey;
	if (regKey.Open(HKEY_CURRENT_USER,L"Software\\IvoSoft\\ClassicStart",KEY_READ|KEY_WRITE)!=ERROR_SUCCESS)
...
{
public:
	DECLARE_WND_CLASS_EX(L"ClassicStartUpdate.COwnerWindow",0,COLOR_MENU)
...
/*
	VersionData data;
	data.Load(L"D:\\Work\\ClassicStart\\ClassicStartSetup\\Final\\update_4.0.4.ver",false);
	return 0;
*/
	...
	Sprintf(mutexName,_countof(mutexName),L"ClassicStartUpdate.Mutex.%s.%s",userName,deskName);
	free(deskName);
