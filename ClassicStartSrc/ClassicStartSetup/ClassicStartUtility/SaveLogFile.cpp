...
static void WriteLogFile( FILE *f )
{
...

	{
		...
		Strcpy(path,_countof(path),L"%ALLUSERSPROFILE%\\ClassicStart");
	...
	Sprintf(fname,_countof(fname),L"%s\\ClassicStartMenu.exe",csPath);
	...
	Sprintf(fname,_countof(fname),L"%s\\ClassicStartUpdate.exe",csPath);
	...
	if (bClassicMenu)
	{
		fwprintf(f,L"  Classic Start Menu\r\n");
	...
	if (bClassicUpdate)
	{
		fwprintf(f,L"  Classic Start Update\r\n\r\n");
	}
			...
			for (int i=0;i<count;i++)
			{
				...
				if (hProcess)
				{
					...
					if (QueryFullProcessImageName(hProcess,0,fname,&size))
					{
						...
						else if (_wcsicmp(name,L"ClassicStartMenu.exe")==0)
						{
...
bool ExtractUtility64( const wchar_t *fname, wchar_t *exe )
{
	...
	if (!hResInfo)
	{
		fwprintf(f,L"Error extracting ClassicStartUtility64.exe\r\n");
		fclose(f);
		return false;
	}
	...
	bool res=false;
	Strcpy(exe,_MAX_PATH,L"%TEMP%\\ClassicStartUtility64.exe");
