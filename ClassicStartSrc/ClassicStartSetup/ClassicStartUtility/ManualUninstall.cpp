...
#include <lm.h>
#include "..\ClassicStartMenu\ClassicStartMenuDLL\ClassicStartMenuDLL.h"
...
// files to delete from the Classic Start folder
static const wchar_t *g_InstalledFiles[]=
{
	...
	L"ClassicIE_64.exe",
	L"ClassicStart.chm",
	L"ClassicStartReadme.rtf",
	L"ClassicStartUpdate.exe",
	L"ClassicStartMenu.exe",
	L"ClassicStartMenuDLL.dll",
	L"ExplorerL10N.ini",
	L"HISTORY.txt",
	L"IE Settings.lnk",
...
// skin files to delete from the Classic Start\Skins folder
...
// shortcuts to delete from the Programs\Classic Start folder
static const wchar_t *g_InstalledShortcuts[]=
{
	L"Classic Explorer Settings.lnk",
	L"Classic IE Settings.lnk",
	L"Classic Start Help.lnk",
	L"Classic Start Readme.lnk",
	L"Classic Start Update.lnk",
	L"Classic Start Menu Settings.lnk",
};
...
LRESULT CUninstallDlg::OnInitDialog( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	{
		HKEY hkey=NULL;
		if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,L"SOFTWARE\\IvoSoft\\ClassicStart",0,KEY_READ|KEY_QUERY_VALUE|KEY_WOW64_64KEY,&hkey)==ERROR_SUCCESS)
		{
			DWORD type=0;
	...
	if (path[0]==0)
	{
		...
		Strcpy(defaultPath,_countof(defaultPath),bWow64?L"%ProgramW6432%\\Classic Start":L"%ProgramFiles%\\Classic Start");
		DoEnvironmentSubst(defaultPath,_countof(defaultPath));
		wchar_t fname1[_MAX_PATH], fname2[_MAX_PATH], fname3[_MAX_PATH];
		Sprintf(fname1,_countof(fname1),L"%s\\ClassicStartMenu.exe",defaultPath);
	...
	if (m_bEmptyPath)
		SetDlgItemText(IDC_STATICPATH,L"The tool failed to locate the Classic Start installation folder. Use the Browse button to locate the correct folder if you want the files in it to be deleted.");
	else
	{
...
LRESULT CUninstallDlg::OnBrowse( WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled )
{
	...
	{
		pDialog->SetTitle(L"Locate the Classic Start install folder");
...
LRESULT CResultsDlg::OnInitDialog( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	if (m_bErrors)
	{
		SetDlgItemText(IDC_STATICRESULT,L"The Classic Start removal tool encountered some errors. Please, restart your computer and try again. If the problem is not resolved"
			L" seek help in the Classic Start forums: www.classicshell.net/forum. Copy the following report and post it in the forum. The report is also saved in a file ClassicStartReport.txt on your desktop.");
	}
	else if (m_bReboot)
	{
		SetDlgItemText(IDC_STATICRESULT,L"The Classic Start removal tool finished. A restart is required to complete the process."
			L"Copy the following report for your records. The report is also saved in a file ClassicStartReport.txt on your desktop.");
	}
	else
	{
		SetDlgItemText(IDC_STATICRESULT,L"The Classic Start removal tool finished."
			L"Copy the following report for your records. The report is also saved in a file ClassicStartReport.txt on your desktop.");
	}
...
{
	wchar_t path[_MAX_PATH]=L"%USERPROFILE%\\Desktop\\ClassicStartReport.txt";
...
static bool RemoveRegistryKeys( bool bPin )
{
	...
	if (error==ERROR_SUCCESS)
	{
		...
		if (RegOpenKeyEx(HKEY_CLASSES_ROOT,L"CLSID\\" EXPLORER_CLSID L"\\" EMULATION_KEY,0,KEY_READ|KEY_WOW64_64KEY,&hkey)==ERROR_SUCCESS)
		{
			...
			MessageBox(NULL,message,L"Classic Start Uninstaller",MB_OK|MB_ICONERROR);
			return false;
		}
	}
...
static void DeleteSettings( HKEY root, const wchar_t *rootName, const wchar_t *keyName )
{
	...
	if (error==ERROR_SUCCESS)
	{
		...
		error=RegDeleteTree2(hkey,L"ClassicStart");
		if (error==ERROR_SUCCESS)
			LogMessage(-1,L"Deleting registry key %s\\%s\\ClassicStart",rootName,keyName);
		else if (error!=ERROR_FILE_NOT_FOUND)
			LogMessage(error,L"Failed to delete registry key %s\\%s\\ClassicStart.",rootName,keyName);
		error=RegDeleteTree2(hkey,L"ClassicStartMenu");
		if (error==ERROR_SUCCESS)
			LogMessage(-1,L"Deleting registry key %s\\%s\\ClassicStartMenu",rootName,keyName);
		else if (error!=ERROR_FILE_NOT_FOUND)
			LogMessage(error,L"Failed to delete registry key %s\\%s\\ClassicStartMenu.",rootName,keyName);
		RegCloseKey(hkey);
...
static void ManualUninstallInternal( void )
{
	...
	progress.SetWindowPos(HWND_TOP,0,0,0,0,SWP_NOSIZE|SWP_NOMOVE);
	{
		...
		wchar_t path[_MAX_PATH];
		Sprintf(path,_countof(path),L"%s\\ClassicStartMenu.exe",dlg.GetPath());
	...
	LogMessage(-1,L"Classic Start Utility - removing Classic Start from the system\r\n\r\n");
	...
	DeleteRegValueSOFTWARE(L"Microsoft\\Windows\\CurrentVersion\\Run",L"Classic Start Menu",bIsWow64);

	DeleteInstallerKey(HKEY_CLASSES_ROOT,L"HKEY_CLASSES_ROOT",L"Installer\\Features",L"ClassicStart",L"");
	DeleteInstallerKey(HKEY_CLASSES_ROOT,L"HKEY_CLASSES_ROOT",L"Installer\\Products",L"ProductName",L"Classic Start");
	DeleteInstallerKey(HKEY_LOCAL_MACHINE,L"HKEY_LOCAL_MACHINE",L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Uninstall",L"DisplayName",L"Classic Start");

	if (dlg.GetRemoveUserSettings() || dlg.GetRemoveAllUserSettings())
	{
		// delete settings for the current user
		LogMessage(-1,L"Deleting user settings from HKEY_CURRENT_USER\\Software\\IvoSoft");
		DeleteSettings(HKEY_CURRENT_USER,L"HKEY_CURRENT_USER",L"Software\\IvoSoft");

		wchar_t local[_MAX_PATH]=L"%LOCALAPPDATA%\\ClassicStart";
		DoEnvironmentSubst(local,_countof(local));
		wchar_t roaming[_MAX_PATH]=L"%APPDATA%\\ClassicStart";
	...
	if (dlg.GetRemoveAllUserSettings())
	{
		...
		for (std::vector<CString>::const_iterator it=userNames.begin();it!=userNames.end();++it)
		{
			...
			Sprintf(local,_countof(local),L"%s\\%s\\AppData\\Local\\ClassicStart",usersFolder,*it);
			wchar_t roaming[_MAX_PATH];
			Sprintf(roaming,_countof(roaming),L"%s\\%s\\AppData\\Roaming\\ClassicStart",usersFolder,*it);
			DeleteUserFiles(local,roaming);
			...
			if (error==ERROR_SUCCESS)
			{
				DeleteSettings(HKEY_USERS,L"HKEY_USERS",L"CSUTempUser\\Software\\IvoSoft");
	...
	if (dlg.GetRemoveAdminSettings())
	{
		...
		LogMessage(-1,L"Deleting settings from HKEY_LOCAL_MACHINE\\Software\\IvoSoft");
		DeleteSettings(HKEY_LOCAL_MACHINE,L"HKEY_LOCAL_MACHINE",L"Software\\IvoSoft");
	}

	// delete program data
	{
		wchar_t dataPath[_MAX_PATH]=L"%ALLUSERSPROFILE%\\ClassicStart";
	...
	// delete start menu shortcuts
	{
		...
		if (SUCCEEDED(SHGetKnownFolderPath(FOLDERID_CommonPrograms,0,NULL,&pPath)))
		{
			...
			Sprintf(startMenuPath,_countof(startMenuPath),L"%s\\Classic Start",(const wchar_t*)pPath);
...
///////////////////////////////////////////////////////////////////////////////
	...
	{
		MessageBox(NULL,L"It is important that you restart as soon as possible. If you attempt to install another software before then it may get corrupted.",L"Classic Start Uninstaller",MB_OK|MB_ICONWARNING);
	}
}
