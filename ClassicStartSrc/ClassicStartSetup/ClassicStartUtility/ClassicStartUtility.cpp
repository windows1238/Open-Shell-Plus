...
int CalcMsiChecksum( wchar_t *const *params, int count )
{
	...
	Sprintf(path1,_countof(path1),L"%s\\ClassicStartSetup32.msi",params[1]);
	LoadFile(path1,buf1);
	if (buf1.empty())
	{
	...
	Sprintf(path2,_countof(path2),L"%s\\ClassicStartSetup64.msi",params[1]);
	LoadFile(path2,buf2);
	if (buf2.empty())
	{
		Printf("Failed to open file %s\n",path2);
...
int MakeEnglishDll( wchar_t *const *params, int count )
{
	...
	// get strings and dialog from ClassicStartUpdate.exe
...
// Classic Start utility - multiple utilities for building and maintaining Classic Start
// Usage:
...
int WINAPI wWinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpstrCmdLine, int nCmdShow )
{
	...
	if (!*lpstrCmdLine)
	{
		...
		TASKDIALOG_BUTTON taskButtons[4]={
			...
			{102,L"Classic Start logging options\nEnable or disable various logging options."},
			{103,L"Remove Classic Start\nManually removes Classic Start from your system. Use only if the conventional methods for uninstallation do not work."},
		...
		tasks.pszWindowTitle=L"Classic Start Utility";
