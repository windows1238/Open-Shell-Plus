...
*/
	...
	if (newestProgram.QuadPart || newestApp.QuadPart)
	{
		CRegKey regKey;
		ULONGLONG val1, val2;
		if (regKey.Open(HKEY_CURRENT_USER,L"Software\\Passionate-Coder\\ClassicStartMenu",KEY_READ)==ERROR_SUCCESS)
		{
...
void CItemManager::LoadOldItems( void )
{
	...
	CRegKey regItems;
	if (regItems.Open(HKEY_CURRENT_USER,L"Software\\Passionate-Coder\\ClassicStartMenu",KEY_READ)==ERROR_SUCCESS)
	{
		ULONG size=0;
...
void CItemManager::AddOldItems( const std::vector<unsigned> &hashes )
{
	...
	CRegKey regItems;
	if (regItems.Open(HKEY_CURRENT_USER,L"Software\\Passionate-Coder\\ClassicStartMenu")!=ERROR_SUCCESS)
		regItems.Create(HKEY_CURRENT_USER,L"Software\\Passionate-Coder\\ClassicStartMenu");
...
void CItemManager::LoadCacheFile( void )
{
	...
	wchar_t path[_MAX_PATH]=L"%LOCALAPPDATA%\\ClassicStart\\DataCache.db";
...
{
	CItemManager *pThis=(CItemManager*)param;
	wchar_t path[_MAX_PATH]=L"%LOCALAPPDATA%\\ClassicStart";
	DoEnvironmentSubst(path,_MAX_PATH);
	...
	{
		wchar_t path3[_MAX_PATH]=L"%LOCALAPPDATA%\\ClassicStart\\DataCache.txt";
	...
	CloseHandle(file);
	wchar_t path2[_MAX_PATH]=L"%LOCALAPPDATA%\\ClassicStart\\DataCache.db";
