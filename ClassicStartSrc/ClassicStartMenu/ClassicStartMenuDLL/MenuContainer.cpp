...
LRESULT CMenuContainer::OnTimer( UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled )
{
	...
	if (wParam==TIMER_DRAG)
	{
		if (!s_bDragClosed)
		{
			...
			if (_wcsicmp(name,L"ClassicStart.CMenuContainer")!=0)
			{
...
void CMenuContainer::SaveMRUShortcuts( void )
{
	Assert(s_bMRULoaded);
	CRegKey regMRU;
	if (regMRU.Open(HKEY_CURRENT_USER,L"Software\\IvoSoft\\ClassicStartMenu\\MRU",KEY_READ|KEY_WRITE)!=ERROR_SUCCESS)
...
void CMenuContainer::LoadMRUShortcuts( void )
{
	for (int i=0;i<MRU_PROGRAMS_COUNT;i++)
		s_MRUShortcuts[i].Empty();
	CRegKey regMRU;
	if (regMRU.Open(HKEY_CURRENT_USER,L"Software\\IvoSoft\\ClassicStartMenu\\MRU",KEY_READ)==ERROR_SUCCESS)
	{
		for (int i=0;i<MRU_PROGRAMS_COUNT;i++)
		{
			wchar_t name[10];
...
HWND CMenuContainer::ToggleStartMenu( int taskbarId, bool bKeyboard, bool bAllPrograms )
{
	...
	{
		CRegKey regKey;
		if (regKey.Open(HKEY_CURRENT_USER,L"Software\\IvoSoft\\ClassicStartMenu\\Settings",KEY_READ|KEY_WOW64_64KEY)==ERROR_SUCCESS)
		{
			DWORD log;
	...
	s_MetroLinkFormat=RegisterClipboardFormat(L"ClassicStart.MetroLink");
